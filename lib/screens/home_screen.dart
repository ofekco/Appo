...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    userBusinessesInstance.ClientId =
        Provider.of<Authentication>(context, listen: false).currentUser.userId;
    userBusinessesInstance.getAllBusinesses();

    userBusinessesInstance.getFavorites();
    userBusinessesInstance
        .getMyUpComingBookings(); //change the id according to the id of the user
    super.initState();
  }
    ...
    if (userBusinessesInstance.MyBookings.isNotEmpty) {
      userBusinessesInstance.MyBookings.forEach((appo) {
        if (!(appo.date.isBefore(DateTime.now()))) {
          ...
          res.add(WrapInkWell(
              MyNextItem(appo, bis), () => itemClicked(context, bis)));
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: ChangeNotifierProvider.value(
        //listen to changes in Businesses
        value: typesProvider,
        child: Column(
          children: [
            SizedBox(
                    ...
                    userBusinessesInstance.Favorites.length < 1
                        ...
                        : ListView.builder(
                            itemBuilder: (ctx, index) => WrapInkWell(
                                FavoriteItem(
                                    userBusinessesInstance.Favorites[index]),
                                () => itemClicked(ctx,
                                    userBusinessesInstance.Favorites[index])),
