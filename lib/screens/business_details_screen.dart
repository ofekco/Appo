...
class _BusinessDetailsScreenState extends State<BusinessDetailsScreen> {
  ...
  void sectionButtonPreesed(int index) {
    setState(() {
      for (int i = 0;
          i < widget.sectionButtonsList.length;
          i++) //set all buttons to unPressed color
      {
        widget.sectionButtonsList[i].state.IsPressed = false;
      }
      _selectedSection = index;
      widget.sectionButtonsList[index].state.IsPressed =
          true; //set the pressed button to pressed color
    });
  }
  ...
  Widget showSelectedSection() {
    ...
    } else if (_selectedSection == 1) //location
    {
      return GoogleMapsView(
          widget.business.latitude, widget.business.longitude);
  ...
  Widget aboutSection() {
    return Container(
      ...
      Row(
        ...
        children: [
          Text(widget.business.phoneNumber, style: TextStyle(fontSize: 14)),
          SizedBox(
            width: 15,
          ),
