...
class _BusinessCalendarScreenState extends State<BusinessCalendarScreen> {
  ...
  Future<void> getSlots() async {
    final url = Uri.parse(
        'https://appo-ae26e-default-rtdb.firebaseio.com/businesses/${businessID}/times.json');
    try {
    ...
    for (var timeKey in json.keys) {
      ...
      if (json[timeKey] != null) {
        if (json[timeKey]['isBooked'] as bool == true) {
          ...
          Appointment appo = Appointment(
              startTime: t.startTime, endTime: t.endTime, client: c);
          createEvent(appo);
        }
      }
    }
  ...
  void createEvent(Appointment booking) {
    CalendarEventData<Appointment> event = CalendarEventData<Appointment>(
